...
scheduler.py — Sequential dynamic nightly pipeline
═══════════════════════════════════════════════════════════════════════════════
v10.0 change: _invalidate_forecast_cache now also resets savi_count so
              main.py reloads the SAVI dataset on the next forecast call.
"""
...
def _stage5_geoserver(generate_callback: Optional[Callable] = None) -> bool:
    """
    Reassign numbered raster names, delete the file that fell out of the
    window, and push new rasters to GeoServer coverage stores.
    """
    logger.info("▶ Stage 5 — GeoServer raster update")
    try:
        if generate_callback is not None:
...
def _invalidate_forecast_cache() -> None:
    """
    Tell main.py to reload both SAVI and PET datasets on the next forecast call.
    v10.0: savi_count added alongside pet_count.
    """
    try:
        import main as _main
        _main._fc_cache["pet_count"]  = -1
        _main._fc_cache["savi_count"] = -1   # v10.0: reset SAVI cache too
        logger.debug("  Forecast dataset cache invalidated (PET + SAVI).")
    except Exception:
        pass
...
def run_nightly_pipeline(
    ...
    Flow (ALL five stages always run):
      Stage 1 — Sentinel check + direct download
      Stage 2 — MOSDAC order placement → returns order_keys
      Stage 3 — MOSDAC download (SFTP) with order_keys
      Stage 4 — Incremental processing (SAVI → Kc → ETc → CWR → IWR)
      Stage 5 — GeoServer raster renaming + update
    """
